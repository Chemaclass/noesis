...
1. **Type** — add the name to the `TActivationName` union in `src/core/types.ts`.
2. **Implementation** — add the function to the `ACTIVATIONS` registry in
   `src/core/activations.ts`. The `as const satisfies Record<TActivationName, …>`
   guard makes the compiler enforce that every name has an implementation.
3. **Test** — add a case to `src/core/activations.test.ts` with a hand-computed
